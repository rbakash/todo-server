  ...
  try {
    ...
    database.query(insertQuery, (error, results, fields) => {
      ...
      res
        .status(200)
        .json({
          message: "New todo list task added",
...
exports.getItem = async (req, res) => {
  try {
    ...
    database.query(retrieveQuery, (error, results, fields) => {
      ...
      res
        .status(200)
        .json({ message: "All the To do list tasks", data: results });
    });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: error.message });
...
exports.updateItem = async (req, res) => {
  try {
    ...
    let updateItemQuery = `UPDATE tbl_todo SET title = ${database.escape(
      req.body.title
    )}
...
exports.deleteItem = async (req, res) => {
  try {
    ...
    let retrieveQuery = `DELETE FROM tbl_todo WHERE (id = ${database.escape(
      req.params.task_Id
    )});`;
    ...
    database.query(retrieveQuery, (error, results, fields) => {
      ...
      res
        .status(200)
        .json({ message: "Deleted the To do list task", data: [] });
    });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: error.message });
